...
const ArchiveMagazinePreview = ({ name, border, color, bgColor, pdfSrc }) => {
  ...
  return (
    ...
    > 
      ...
      <div className="max-w-96 h-96  mx-auto">
        <div className="pt-10 w-full   border-3 border-black">
          <Image height={256} width={256} src="/books.jpg" alt="Image" className="w-full h-full border border-black" />
        </div>
        <div className="h-auto flex justify-center mt-4">
          <button onClick={handleCistClick} className="btn bg-secondary-color">
            ČÍST
          </button>
          <button className="btn bg-secondary-color ml-2">KOMIKS</button>
        </div>
        <div div className="ml-auto w-full mt-4 text-right">2022/2023</div>
      </div>
    </div>
  );
};
